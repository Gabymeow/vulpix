...
function setupCropCanvas() {
  requestAnimationFrame(() => {
    ...
    // Размер контейнера = отображаемый размер
    ...
    // ВАЖНО: canvas рисуем в ПОЛНОМ разрешении
    canvas.width = cropImg.width;
    canvas.height = cropImg.height;
    
    // А масштабируем через CSS
...
function drawCrop() {
  ...
  // Canvas теперь в полном разрешении
  const CW = canvas.width;   // = cropImg.width
  const CH = canvas.height;  // = cropImg.height
  
  ctx.clearRect(0, 0, CW, CH);
  
  // Рисуем изображение в полном размере
  ctx.drawImage(cropImg, 0, 0, CW, CH);

  // Координаты рамки в полном разрешении (без масштаба!)
  const rx = cropRect.x;
  const ry = cropRect.y;
  const rw = cropRect.w;
  const rh = cropRect.h;
  ...
  ctx.lineWidth = 2 / cscale;  // Толщина линии с учётом масштаба отображения
  ...
  // Угловые маркеры
  const hs = 12 / cscale;  // Размер с учётом масштаба
  ctx.fillStyle = 'white';
...
function getCanvasPos(e) {
  ...
  // Координаты в пикселях отображения
  const displayX = cx - r.left;
  const displayY = cy - r.top;
  
  // Преобразуем в координаты полного разрешения
  return {
    x: displayX / cscale,
    y: displayY / cscale
  };
...
function getHitMode(mx, my) {
  // mx, my теперь уже в координатах полного разрешения
  const rx = cropRect.x;
  const ry = cropRect.y;
  const rw = cropRect.w;
  const rh = cropRect.h;

  // Размер зон захвата в координатах полного разрешения
...
// ВСПОМОГАТЕЛЬНАЯ ФУНКЦИЯ: ограничение размеров с сохранением соотношения
function constrainRect(x, y, w, h, IW, IH) {
  // Сначала ограничиваем размеры
  ...
  if (activeRatio.w && activeRatio.h) {
    ...
    if (Math.abs(currentRatio - r) > 0.01) {
      // Определяем, что ограничивает: ширина или высота
      const maxWByH = Math.min(IW, h * r);
      const maxHByW = Math.min(IH, w / r);
      
      if (maxWByH <= IW && maxHByW <= IH) {
        // Оба варианта подходят - выбираем ближайший к текущему
