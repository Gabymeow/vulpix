...
function setupCropCanvas() {
  // Ждём один кадр чтобы DOM точно отрисовался и clientWidth был корректным
  requestAnimationFrame(() => {
    ...
    const maxH = Math.min(window.innerHeight * 0.60, 650);
...
// ── Настройки зон взаимодействия ──────────────────────────────
// Увеличь HS  — если трудно попасть на угол
// Увеличь EDGE — если трудно попасть на грань
// MOVE_MARGIN — доля от размера рамки с каждой стороны, где работает перемещение
//   0.25 = центральные 50% рамки (по 25% отступ с каждой стороны)
//   0.4  = центральные 20% рамки (труднее захватить, но меньше конфликтов с гранями)
//   0    = вся внутренняя часть рамки — перемещение (грани работать не будут!)
const HS          = 18;   // радиус захвата угла в пикселях экрана
const EDGE        = 12;   // ширина полосы захвата грани в пикселях экрана
const MOVE_MARGIN = 0.01; // доля рамки с каждой стороны для зоны перемещения
// ──────────────────────────────────────────────────────────────
...
function getHitMode(mx, my) {
  ...
  const hitCorner = corners.find(c => Math.abs(mx - c.x) < HS && Math.abs(my - c.y) < HS);
  ...
  // 2. Грани (только если курсор внутри или чуть за рамкой)
  const nearTop    = Math.abs(my - ry)      < EDGE && insideX;
  const nearBottom = Math.abs(my - (ry+rh)) < EDGE && insideX;
  const nearLeft   = Math.abs(mx - rx)      < EDGE && insideY;
  const nearRight  = Math.abs(mx - (rx+rw)) < EDGE && insideY;
  if (nearTop)    return 'edge-n';
  if (nearBottom) return 'edge-s';
  if (nearLeft)   return 'edge-w';
  if (nearRight)  return 'edge-e';

  // 3. Центральная зона — перемещение
  if (inside) {
  ...
  // 4. Остальное — рисуем новую рамку
  return 'new';
}

// Курсор меняется в зависимости от зоны под ним
const CURSORS = {
...
cropCanvas.addEventListener('mousemove', e => {
  if (dragging) return; // во время drag курсор не трогаем
  const {x: mx, y: my} = getCanvasPos(e);
  const mode = getHitMode(mx, my);
  cropCanvas.style.cursor = CURSORS[mode] || 'crosshair';
});
cropCanvas.addEventListener('mouseleave', () => {
  if (!dragging) cropCanvas.style.cursor = 'crosshair';
});
...
cropCanvas.addEventListener('mousedown', e => {
  ...
  dragStart = {x: mx, y: my};
  if (mode === 'new') {
    cropRect = {x: Math.round(mx / cscale), y: Math.round(my / cscale), w: 0, h: 0};
  }
  imgRect0 = {...cropRect};
  e.preventDefault();
});
...
window.addEventListener('mousemove', e => {
  ...
  if (dragMode === 'move') {
    ...
    // Грань: двигаем только одну сторону
    let {x, y, w, h} = imgRect0;
    ...
    if (side === 'w') { x += dx; w -= dx; }
    if (side === 'e') { w += dx; }
    // Если зафиксировано соотношение — подтягиваем вторую ось
    if (activeRatio.w && activeRatio.h) {
      const r = activeRatio.w / activeRatio.h;
      if (side === 'n' || side === 's') { w = Math.round(h * r); } // меняем высоту → корректируем ширину
      else                              { h = Math.round(w / r); } // меняем ширину → корректируем высоту
    }
    if (w > 10 && h > 10) {
      cropRect.x = Math.max(0, Math.min(IW - w, x));
      cropRect.y = Math.max(0, Math.min(IH - h, y));
...
// ─── TOUCH: прямая обработка без эмуляции MouseEvent ───
cropCanvas.addEventListener('touchstart', e => {
  e.preventDefault();
  const t = e.touches[0];
  const {x: mx, y: my} = getCanvasPos(t);
  const mode = getHitMode(mx, my);
  dragging  = true;
  dragMode  = mode;
  dragStart = {x: mx, y: my};
...
window.addEventListener('touchmove', e => {
  ...
  if (dragMode === 'move') {
    ...
    if (activeRatio.w && activeRatio.h) {
      const r = activeRatio.w / activeRatio.h;
      if (side === 'n' || side === 's') { w = Math.round(h * r); }
